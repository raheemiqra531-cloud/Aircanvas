...
class AirSmartBoard(ctk.CTk):
    ...
    def update_video(self):
        ...
        if points:
            ...
            # Smoothing
            self.smooth_x = int(SMOOTHING*cx + (1-SMOOTHING)* (self.smooth_x if self.smooth_x else cx))
            self.smooth_y = int(SMOOTHING*cy + (1-SMOOTHING)* (self.smooth_y if self.smooth_y else cy))
            current = (self.smooth_x, self.smooth_y)

            # Draw
            if True:  # always draw with index finger
                if self.prev_point is None:
                    self.prev_point = current
                dx = current[0] - self.prev_point[0]
                dy = current[1] - self.prev_point[1]
                ...
                if distance < MAX_JUMP_DISTANCE:
                    ...
                    for i in range(steps):
                        ...
                        color = self.color if self.tool=="BRUSH" else (0,0,0)
                        thickness = self.brush_size if self.tool=="BRUSH" else self.brush_size*4
                        cv2.circle(self.canvas_layer,(ix,iy),thickness,color,-1)
                self.prev_point = current
        else:
            self.prev_point = None
        ...
        if self.show_help:
            ...
            y=60
            for line in lines:
                cv2.putText(combined,line,(40,y),
                            cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,255),1)
                y+=30

        # Color preview
        cv2.rectangle(combined, (20,350),(80,410), self.color, -1)

        fps=int(1/(time.time()-start+0.0001))
        cv2.putText(combined,f"FPS: {fps}",
                    (1100,30),
